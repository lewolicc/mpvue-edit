<template>
  <div> 
    <div v-show="isShowAdd">
       <div>
            <button class="mybutton" @click="onAddDetail">添加一个条信息</button>
            <button class="mybutton" @click="onSubmit">确认上传</button>
        </div>
        <ul>
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit () {
        ...
        var token = wx.getStorageSync('token')
        wx.request({
          url: 'https://xiaochengxu.nibaspace.com/commodity/addGoodsDetailList',
          method: 'POST',
          data: {
            bizId: this.id,
          ...
          success: function (res) {
            ...
            if (res.data.result) {
              ...
              that.onLoding('上传成功')
              wx.navigateTo({
                url: '../productChange/main?id=' + that.id
              })
            } else {
              that.onLoding('上传失败')
            }
